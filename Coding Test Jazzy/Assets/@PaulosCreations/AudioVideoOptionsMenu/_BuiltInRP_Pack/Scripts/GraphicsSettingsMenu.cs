...
public class GraphicsSettingsMenu : MonoBehaviour
{
    ...
    [SerializeField]
    private SettingsUsedState resolutionUsed, shadowResolutionUsed, windowedModeUsed, vSyncUsed, antiAliasingUsed, textureQualityUsed, anisotropicModeUsed, anisotropicLvlUsed;

    [Header("Values to use on Reset or if no values are saved")]
    [SerializeField]
    private MenuVariables DefaultSettings = new MenuVariables();

    private MenuVariablesSimple DefaultSettingsConverted = new MenuVariablesSimple();
    private MenuVariablesSimple CurrentSettings = new MenuVariablesSimple();
    ...
    [SerializeField]
    private TMP_Text resolutionText, windowedModeText, shadowResolutionText, vsyncText, antiAliasingText, textureQualityText, anisoFilteringModeText, anisoFilteringLevelText;
    [SerializeField]
    ...
    private void ConvertDefaultSettings()
    {
        ...
        switch (DefaultSettings.ShadowResolution)
        {
            case ShadowResolution.Low:
                DefaultSettingsConverted.ShadowResolution = 0;
                break;
            case ShadowResolution.Medium:
                DefaultSettingsConverted.ShadowResolution = 1;
                break;
            case ShadowResolution.High:
                DefaultSettingsConverted.ShadowResolution = 2;
                break;
            case ShadowResolution.VeryHigh:
                DefaultSettingsConverted.ShadowResolution = 3;
                break;
        }
    ...
    public void UI_SetShadowResolution(int _shadowResolution)
    {
        if (shadowResolutionUsed == SettingsUsedState.notUsed || isApplying)
            return;

        CurrentSettings.ShadowResolution = _shadowResolution;

        switch (CurrentSettings.ShadowResolution)
        {
            case 0:
                QualitySettings.shadowResolution = ShadowResolution.Low;
                shadowResolutionText.text = "low";
                break;
            case 1:
                QualitySettings.shadowResolution = ShadowResolution.Medium;
                shadowResolutionText.text = "Medium";
                break;
            case 2:
                QualitySettings.shadowResolution = ShadowResolution.High;
                shadowResolutionText.text = "Heigh";
                break;
            case 3:
                QualitySettings.shadowResolution = ShadowResolution.VeryHigh;
                shadowResolutionText.text = "Verry Heigh";
                break;
        }
    }
    ...
    public void UI_SetAntiAliasing(int _antiAliaslevel)
    {
        ...
        QualitySettings.antiAliasing = CurrentSettings.AntiAliaslevel;
        ...
        //Text will not be set to "Off" ?????? Works fine with URP ??
    ...
    private void LoadMenuVariables()
    {
        if (saveFormat == SaveFormat.playerprefs)
        {
            ...
            {
                MenuVariablesSimple newMenuVars = new MenuVariablesSimple();
                ...
                newMenuVars.ShadowResolution = PlayerPrefs.GetInt("ShadowResolution");
        ...
        else if (saveFormat == SaveFormat.iniFile)
        {
            ...
            {
                MenuVariablesSimple newMenuVars = JsonUtility.FromJson<MenuVariablesSimple>(File.ReadAllText(saveFileDataPath));
    ...
    private void ApplySettings(MenuVariablesSimple _varsLoaded)
    {
        isApplying = true;
        ...
        {
            QualitySettings.antiAliasing = _varsLoaded.AntiAliaslevel;

            switch (_varsLoaded.AntiAliaslevel)
            {
                case 0:
                    antiAliasingText.text = "Off";
                    break;
            ...
            //Text wont set to "Off" ?????? works with URP ?? O.o
            //if (_varsLoaded.AntiAliaslevel != 0)
            //    antiAliasingText.SetText("{0}x", _varsLoaded.AntiAliaslevel);
            //else antiAliasingText.text = "Off";
        }

        if (shadowResolutionUsed == SettingsUsedState.used)
        {
            switch (_varsLoaded.ShadowResolution)
            {
                case 0:
                    QualitySettings.shadowResolution = ShadowResolution.Low;
                    shadowResolutionText.text = "low";
        ...
        CurrentSettings.ShadowResolution = _varsLoaded.ShadowResolution;
    ...
    private void SaveMenuVariables()
    {
        if (saveFormat == SaveFormat.playerprefs)
        {
            ...
            PlayerPrefs.SetInt("ShadowResolution", CurrentSettings.ShadowResolution);
        ...
        else if (saveFormat == SaveFormat.iniFile)
        {
            ...
            MenuVariablesSimple menuVarsToSave = new MenuVariablesSimple();
            ...
            if (shadowResolutionUsed == SettingsUsedState.used)
                menuVarsToSave.ShadowResolution = CurrentSettings.ShadowResolution;
            else
            {
                switch (QualitySettings.shadowResolution)
                {
                    case ShadowResolution.Low:
                        menuVarsToSave.ShadowResolution = 0;
                        break;
                    case ShadowResolution.Medium:
                        menuVarsToSave.ShadowResolution = 1;
                        break;
                    case ShadowResolution.High:
                        menuVarsToSave.ShadowResolution = 2;
                        break;
                    case ShadowResolution.VeryHigh:
                        menuVarsToSave.ShadowResolution = 3;
                        break;
                }
            }
            ...
            {
                menuVarsToSave.AntiAliaslevel = QualitySettings.antiAliasing;
            }
...
namespace PaulosVideoMenu
{
    ...
    [System.Serializable]
    public class MenuVariablesSimple
    {
